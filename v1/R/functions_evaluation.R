...
cmaq_to_percentile = function(path = "data_raw/cmaq.csv", prob = 0.50, min_cost = 0){
  ...
  file_good = if(!is.null(path)){ file.exists(path) }else{ FALSE }
  ...
  data = cmaq %>%
    ...
    pivot_longer(cols = c(contains("_benefit")), 
                 names_to = 'pollutant', values_to = 'effect_emissions',
                 values_drop_na = TRUE) %>%
    # Recode Pollutant
    mutate(pollutant = pollutant %>% dplyr::recode(
      "pm10_benefit" = 100,
      "co_benefit" = 2,
      "co2_benefit" = 98,
      "nox_benefit" = 3,
    ...
    mutate(effect_emissions = effect_emissions * 0.00110231 * 365.25,
           cost2022 = cost2022)
...
get_avg = function(data, var = "ce", range = 5){
  ...
  avg = data %>%
    ...
    distinct() %>%
    # Get full timespan
    reframe(year = seq(from = min(year, na.rm = TRUE),
    ...
    left_join(by = c("yearrange" = "year", "category_id", "pollutant"),
              y = data, multiple = "all")  %>%
  ...
  lower_year = avg %>% with(year) %>% min(na.rm = TRUE) + extra
  upper_year = avg %>% with(year) %>% max(na.rm = TRUE) - extra
...
get_cost_effectiveness = function(
  ...
  for(i in frame$id){
    ...
    if(is.null(model)){
      insert = frame %>% filter(id == i) %>% mutate(value = NA_real_)
...
get_frame = function(data){
  ...
  frame = data %>%
    ...
    reframe(
      year_range = seq(from = min(year, na.rm = TRUE), to = max(year, na.rm = TRUE), by = 1),
...
sim_diff = function(x0, x1, var = "proxy", model, adjust = -1, n = NULL, fundamental = FALSE){
  ...
  if(is.null(n)){
    ...
    output = data %>% left_join(by = ".id", y = uncertainty) %>%
...
get_effects = function(.geoid = "36109", .pollutants = c(2, 3, 87, 98, 100), var = "epd", path = NULL, prob = 0.50, min_cost = 0, range = 5,  .policy = c(1,2), .start_year = 2020, .end_year = 2025, .units = 30000, fundamental = FALSE, n = 1000){
  ...
  ce = ce %>%
    pivot_wider(id_cols = c(policy,year, affect_vmt, affect_ef, cost), 
  ...
  get_vmt = function(x, model){ predict(object = model, newdata = tibble(emissions = x)) %>% exp() - 1 }
  
  m_ef = training %>% 
    lm(formula = log(ef + 0.001) ~ log(emissions) - 1)
  get_ef = function(x, model){ predict(object = model, newdata = tibble(emissions = x)) %>% exp() - 0.001 }
  ...
  pdata0 = expand_grid(
    ...
    left_join(by = c("policy" = "policy", "year"),
              y = ce) %>%
  ...
  vmt_data = 1:length(valid_pollutants) %>%
    map_dfr(~pdata0 %>%
              ...
              with(sim_diff(x0 = base_emissions, x1 = base_emissions + effect_emissions, var = "emissions", model = m_vmt, 
  ...
  ef_data = 1:length(valid_pollutants) %>%
    map_dfr(~pdata0 %>%
              select(base_emissions = var_base[.x], effect_emissions = var_effect[.x]) %>%
              with(sim_diff(x0 = base_emissions, x1 = base_emissions + effect_emissions, var = "emissions", model = m_ef, 
  ...
  pdata2 = pdata0 %>%
    ...
    mutate(across(.cols = contains("effect_ef"), .fns = ~.x * affect_ef),
  ...
  output = pdata2 %>%
    select(.id, policy, year, affect_vmt, cost,
