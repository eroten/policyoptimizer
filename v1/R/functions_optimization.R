...
#' @importFrom ompr sum_over
get_diversification = function(absolute, n, width, prior_diversification = 0){ sum_over(absolute[i] / width, i = 1:n) + prior_diversification  }
...
get_constants = function(effects, training, n_scales = 100, .year = 2005, .percent_change = 0.50){
  # Testing values
  ...
  pollutants = names(effects)[str_detect(names(effects), "effect_emissions[0-9]+")] %>% 
    str_remove("effect_emissions") %>% as.integer()
  
  if(98 %in% pollutants){
    # Get upper emissions - max emissions over time for that county
    output$ue98 = training %>% filter(pollutant == 98, year >= .year) %>% with(emissions) %>% max()
    # Get lower emissions - should be target a certain percent change less than in specified year
    output$le98 = training %>% filter(pollutant == 98 & year == .year) %>% summarize(target = .percent_change * emissions) %>% with(target)
  ...
  if(100 %in% pollutants){
    # For pollutant 100
    output$ue100 = training %>% filter(pollutant == 100, year >= .year) %>% with(emissions) %>% max()
    output$le100 = training %>% filter(pollutant == 100 & year == .year) %>% summarize(target = .percent_change * emissions) %>% with(target)
  ...
  if(2 %in% pollutants){
    # For pollutant 2
    output$ue2 = training %>% filter(pollutant == 2, year >= .year) %>% with(emissions) %>% max()
    output$le2 = training %>% filter(pollutant == 2 & year == .year) %>% summarize(target = .percent_change * emissions) %>% with(target)
  ...
  if(3 %in% pollutants){
    # For pollutant 3
    output$ue3 = training %>% filter(pollutant == 3, year >= .year) %>% with(emissions) %>% max()
    output$le3 = training %>% filter(pollutant == 3 & year == .year) %>% summarize(target = .percent_change * emissions) %>% with(target)
    output$ur3 = output$ue3 - output$le3
    output$lr3 = 0 
  }
  if(87 %in% pollutants){
    # Repeated
    output$ue87 = training %>% filter(pollutant == 87, year >= .year) %>% with(emissions) %>% max()
    output$le87 = training %>% filter(pollutant == 87 & year == .year) %>% summarize(target = .percent_change * emissions) %>% with(target)
...
get_record = function(geoid = "36109", start_year = 2025, end_year = 2050, units = 30000, policies = 1:10, pollutants = c(2, 3, 87, 98, 100),
                      last_cost = 0, last_reduction98 = 0, last_reduction2 = 0, last_reduction3 = 0, last_reduction87 = 0, last_reduction100 = 0){
  
  
  # Get training data
  training = get_training(.geoid = geoid, .by = "16", .pollutant = pollutants, test = FALSE); 
...
get_optimal = function(
  ...
  # min_emissions_chg100 = NULL; min_annual_emissions_chg100 = NULL; min_annual_ef_chg100 = NULL
  # 
  if(debug == TRUE){ verbose = TRUE }else{ verbose = FALSE}
  ...
  if(.year < max(.record$year)){   .record[ next_index, "prior_cost"] = .cost  }
  ...
  if(98 %in% pollutant){
    ...
    if(.year < max(.record$year)){   .record[ next_index, "prior_reduction98"] = .reduction98 }
  ...
  if(2 %in% pollutant){
    ...
    if(.year < max(.record$year)){  .record[ next_index, "prior_reduction2"] = .reduction2 }
  ...
  if(3 %in% pollutant){
    ...
    if(.year < max(.record$year)){  .record[ next_index, "prior_reduction3"] = .reduction3 }
  ...
  if(100 %in% pollutant){
    ...
    if(.year < max(.record$year)){  .record[ next_index, "prior_reduction100"] = .reduction100 }
  ...
  if(87 %in% pollutant){
    ...
    if(.year < max(.record$year)){  .record[ next_index, "prior_reduction87"] = .reduction87 }
...
policy_optimizer = function(
  ...
  last_cost = 0, last_reduction98 = 0, last_reduction2 = 0,last_reduction3 = 0,last_reduction100 = 0,last_reduction87 = 0,
  ...
  effects = get_effects(.geoid = geoid, .pollutants = pollutants,
                        .policy = policies, .start_year = start_year, .end_year = end_year, .units = units,
                        var = var, prob = prob, min_cost = 0, range = range, path = cmaq_path)
  record = get_record(geoid = geoid, pollutants = pollutants, policies = policies, start_year = start_year, end_year = end_year, units = units, 
                      last_cost = last_cost, last_reduction98 = last_reduction98, last_reduction2 = last_reduction2, last_reduction3 = last_reduction3, last_reduction100 = last_reduction100, last_reduction87 = last_reduction87)
  constants = get_constants(effects = effects, training = training, n_scales = n_scales, .year = target_year, .percent_change = target_change)
  ...
  if(is.null(max_annual_policies)){ record$max_annual_policies = length(policies) }else{ record$max_annual_policies  = max_annual_policies }
  # The minimum annual number of policies is 0
  if(is.null(min_annual_policies)){ record$min_annual_policies = 0 }else{ record$min_annual_policies = min_annual_policies }
  # The minimum number of total policies is 1
  if(is.null(min_total_policies)){ record$min_total_policies = 1 }else{ record$min_total_policies = min_total_policies }
  # The max total policies is 
  if(is.null(max_total_policies)){ record = record %>% mutate(max_total_policies = max_annual_policies * n_years) }else{ record$max_total_policies = min_total_policies }
  ...
  if(!is.null(max_total_cost)){ record$max_total_cost = max_total_cost }
  ...
  if(!is.null(max_annual_cost)){ record$max_annual_cost = max_annual_cost }
  
  
  # Set minimum annual cost to 0
  record$min_annual_cost = 0 
  # If supplied, overwrite it
  if(!is.null(min_annual_cost)){ record$min_annual_cost = min_annual_cost}
  
  # set minimum total cost to 0
  record$min_total_cost = 0
  # If supplied, overwrite it.
  if(!is.null(min_total_cost)){ record$min_total_cost = min_total_cost}
  ...
  if(check_cost == FALSE){  stop("cumulative sum of min_annual_cost does not add up at least min_total_cost for at least 1 year.") }
  
  
  if(98 %in% pollutants){
    record$min_emissions_chg98 = 0
    if(!is.null(min_emissions_chg98)){ record$min_emissions_chg98 = min_emissions_chg98  }else{   }
    record$min_annual_emissions_chg98 = 0
    if(!is.null(min_annual_emissions_chg98)){ record$min_annual_emissions_chg98 = min_annual_emissions_chg98 }
    
    # Minimum total cost by each year must always be less than or equal to the cumulative sum of the min annual cost
    check_reduction98 = all(cumsum(record$min_annual_emissions_chg98) >= record$min_emissions_chg98)
    if(check_reduction98 == FALSE){  stop("cumulative sum of min_emissions_chg98 does not add up at least min_emissions_chg98 for at least 1 year.") }
    
  }
  if(100 %in% pollutants){
    record$min_emissions_chg100 = 0
    if(!is.null(min_emissions_chg100)){ record$min_emissions_chg100 = min_emissions_chg100  }
    record$min_annual_emissions_chg100 = 0
    if(!is.null(min_annual_emissions_chg100)){ record$min_annual_emissions_chg100 = min_annual_emissions_chg100 }
    
    # Minimum total cost by each year must always be less than or equal to the cumulative sum of the min annual cost
    check_reduction100 = all(cumsum(record$min_annual_emissions_chg100) >= record$min_emissions_chg100)
    if(check_reduction100 == FALSE){  stop("cumulative sum of min_emissions_chg100 does not add up at least min_emissions_chg100 for at least 1 year.") }
    
  }
  if(2 %in% pollutants){
    record$min_emissions_chg2 = 0
    if(!is.null(min_emissions_chg2)){ record$min_emissions_chg2 = min_emissions_chg2  }
    record$min_annual_emissions_chg2 = 0
    if(!is.null(min_annual_emissions_chg2)){ record$min_annual_emissions_chg2 = min_annual_emissions_chg2 }
    ...
    if(check_reduction2 == FALSE){  stop("cumulative sum of min_emissions_chg2 does not add up at least min_emissions_chg2 for at least 1 year.") }
    
  }
  if(3 %in% pollutants){
    record$min_emissions_chg3 = 0
    if(!is.null(min_emissions_chg3)){ record$min_emissions_chg3 = min_emissions_chg3  }
    record$min_annual_emissions_chg3 = 0
    if(!is.null(min_annual_emissions_chg3)){ record$min_annual_emissions_chg3 = min_annual_emissions_chg3 }
    
    # Minimum total reduction by each year must always be less than or equal to the cumulative sum of the min annual reduction
    check_reduction3 = all(cumsum(record$min_annual_emissions_chg3) >= record$min_emissions_chg3)
    if(check_reduction3 == FALSE){  stop("cumulative sum of min_emissions_chg3 does not add up at least min_emissions_chg3 for at least 1 year.") }
    
  }
  if(87 %in% pollutants){
    record$min_emissions_chg87 = 0
    if(!is.null(min_emissions_chg87)){ record$min_emissions_chg87 = min_emissions_chg87  }
    record$min_annual_emissions_chg87 = 0
    if(!is.null(min_annual_emissions_chg87)){ record$min_annual_emissions_chg87 = min_annual_emissions_chg87 }
    
    # Minimum total reduction by each year must always be less than or equal to the cumulative sum of the min annual reduction
    check_reduction87 = all(cumsum(record$min_annual_emissions_chg87) >= record$min_emissions_chg87)
    if(check_reduction87 == FALSE){  stop("cumulative sum of min_emissions_chg87 does not add up at least min_emissions_chg87 for at least 1 year.") }
  ...
  for(i in 1:length(record$year) ){
    ...
    record = effects %>%
      ...
      get_optimal(.record = record, .constants = constants, .effects = .,
                  debug = FALSE,
                  type = "integer",
                  wc = wc, wr = wr, 
  ...
  if(any("error" %in% record$status)){ message("Some years in scenario were not optimized successfully.")}
  return(record)
}
