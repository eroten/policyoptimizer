...
summarize_by_policy <- function(record, path = NULL) {
  ...
  # Get effects data
  effects <- get_effects(
    .geoid = .geoid, .pollutants = pollutants, .start_year = .start_year,
    .end_year = .end_year, .units = .units,
  ...
  output <- record %>%
    ...
    summarize(
      across(
        any_of(c("effect_cost", "effect_emissions", 
                 effect_vars, "effect_vmt", "effect_ef", ef_vars)),
        ~ sum(.x * scale * units)
      ),
      .groups = "drop"
    ) %>%
...
summarize_by_year <- function(record) {
  ...
  for (i in pollutants) {
    ...
    record <- record %>%
      mutate(
        !!sym(vars_emissions) := !!sym(vars_base_emissions) + cumsum(!!sym(vars_effect_emissions)),
        !!sym(vars_ef) := !!sym(vars_base_ef) + cumsum(!!sym(vars_effect_ef))
      )
  }

  pollutant_vars <- c(
    paste0("effect_emissions", pollutants),
  ...
  output <- record %>%
    ...
    select(
      any_of(c("geoid")), year, base_cost, effect_cost, cost,
...
visualize_by_policy <- function(data, var = "effect_cost", palette = "mako", adjust = 100) {
  ...
  gg <- ggplot() +
    ...
    geom_col(
      data = data, mapping = aes(x = reorder(policy_name, -policy), y = stat),
      color = "#373737", fill = color
    ) +
    geom_shadowtext(
      data = data,
      mapping = aes(
        x = reorder(policy_name, -policy), y = stat,
...
visualize_by_year <- function(data, var = "cost", palette = "mako", adjust = 100) {
  ...
  text <- data %>%
    ...
    mutate(
      label_stat = number(stat, prefix = prefix, scale_cut = cut_si(unit)),
  ...
  gg <- ggplot() +
    geom_ribbon(
      data = data, mapping = aes(x = year, ymin = base, ymax = stat),
      fill = color, alpha = 0.8, color = color, linewidth = 2
    ) +
    geom_line(
      data = data, mapping = aes(x = year, y = stat),
      linewidth = 1.5, linetype = "dashed", color = "black"
    ) +
    geom_point(
      data = text, mapping = aes(x = year, y = base),
      shape = 21, fill = color, color = color, size = 5
    ) +
    geom_point(
      data = text, mapping = aes(x = year, y = stat),
    ...
    scale_y_continuous(
      labels = label_number(prefix = prefix, scale_cut = cut_si(unit)),
